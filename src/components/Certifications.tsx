...
import { Award, ExternalLink, ShieldCheck, CheckCircle, Bookmark, FolderArchive } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {data.certifications.map((cert) => (
            ...
            >
              ...
              {cert.imageUrl && (
                <div className="relative aspect-[16/10] overflow-hidden bg-[#FAF8F5] border-b border-[#EAE4DB]">
                  <img
                    src={cert.imageUrl}
                    alt={cert.title}
                  ...
                  <div className="absolute top-3 right-3 px-2.5 py-1 rounded-full bg-white/90 backdrop-blur-md border border-[#E2D9CC] text-[10px] font-mono-code text-[#201D1A] flex items-center gap-1 shadow-2xs">
                    <CheckCircle className="w-3 h-3 text-[#9A7B61]" />
                    <span>Verified</span>
                  </div>
                </div>
              )}
